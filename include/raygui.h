...
RAYGUIAPI bool GuiTextBoxV2(
    Rectangle bounds, char *text, Font font, int fontSize, int textSize,
...
//----------------------------------------------------------------------------------
static int GetTextWidthV2(Font font, const char *text, int fontSize);
...
Rectangle GuiScrollPanel(Rectangle bounds, const char *text, Rectangle content,
    ...
    if (hasHorizontalScrollBar && hasVerticalScrollBar) {
        ...
        GuiDrawRectangle(
            corner, 0, BLANK,
            Fade(GetColor(GuiGetStyle(LISTVIEW, TEXT + (state * 3))),
                 guiAlpha));
    }
    ...
    if (state == STATE_NORMAL) {
        GuiDrawRectangle(
            ...
            Fade(GetColor(GuiGetStyle(TOGGLE, (active ? BORDER_COLOR_PRESSED
                                                      : (BORDER + state * 3)))),
                 guiAlpha),
            Fade(GetColor(GuiGetStyle(TOGGLE, (active ? BASE_COLOR_PRESSED
                                                      : (BASE + state * 3)))),
                 guiAlpha));
        GuiDrawText(
            text, GetTextBounds(TOGGLE, bounds),
            GuiGetStyle(TOGGLE, TEXT_ALIGNMENT),
            Fade(GetColor(GuiGetStyle(TOGGLE, (active ? TEXT_COLOR_PRESSED
                                                      : (TEXT + state * 3)))),
                 guiAlpha));
    } else {
        GuiDrawRectangle(
            ...
            for (int i = 0; i < itemCount; i++) {
                ...
                if (CheckCollisionPointRec(mousePoint, itemBounds)) {
                    ...
                    if (IsMouseButtonReleased(MOUSE_LEFT_BUTTON)) {
                        itemSelected = i;
                        pressed =
                            true;  // Item selected, change to editMode = false
                    }
                    break;
                }
            }
        ...
        for (int i = 0; i < itemCount; i++) {
            ...
            if (i == itemSelected) {
                GuiDrawRectangle(
                    ...
                    Fade(GetColor(
                             GuiGetStyle(DROPDOWNBOX, BORDER_COLOR_PRESSED)),
                         guiAlpha),
                    Fade(GetColor(GuiGetStyle(DROPDOWNBOX, BASE_COLOR_PRESSED)),
                         guiAlpha));
                GuiDrawText(
                    items[i], GetTextBounds(DEFAULT, itemBounds),
                    GuiGetStyle(DROPDOWNBOX, TEXT_ALIGNMENT),
                ...
                GuiDrawRectangle(
                    itemBounds, GuiGetStyle(DROPDOWNBOX, BORDER_WIDTH),
                    Fade(GetColor(
                             GuiGetStyle(DROPDOWNBOX, BORDER_COLOR_FOCUSED)),
                         guiAlpha),
...
bool GuiTextBoxV2(Rectangle bounds, char *text, Font font, int fontSize,
                  int textSize, bool editMode) {
    GuiState state = guiState;
    bool pressed = false;

    Rectangle cursor = {bounds.x + GuiGetStyle(TEXTBOX, TEXT_PADDING) +
                            GetTextWidthV2(font, text, fontSize) + 2,
                        bounds.y + bounds.height / 2 - fontSize, 4,
                        (float)fontSize * 2};
    ...
    if ((state != STATE_DISABLED) && !guiLocked) {
        ...
        if (editMode) {
            ...
            if (key >= 128) {
                keyCount--;
                if (text[keyCount] & 0x80) {
                    while ((text[keyCount] & 0xc0) == 0x80) {
                        text[keyCount--] = '\0';
                    }
                    text[keyCount] = '\0';
                } else
            ...
            if ((keyCount + byteSize) < textSize) {
                ...
                if ((GetTextWidthV2(font, text, fontSize) <
                     (maxWidth - fontSize)) &&
                    (key >= 32)) {
            ...
            if (textAlignment == TEXT_ALIGN_CENTER)
                cursor.x = bounds.x + GetTextWidthV2(font, text, fontSize) / 2 +
                           bounds.width / 2 + 1;
    ...
    if (state == STATE_PRESSED) {
        ...
        GuiDrawRectangle(
            ...
            Fade(GetColor(GuiGetStyle(TEXTBOX, BORDER + (state * 3))),
                 guiAlpha),
            Fade(GetColor(GuiGetStyle(TEXTBOX, BASE_COLOR_DISABLED)),
                 guiAlpha));
    } else
        GuiDrawRectangle(
            bounds, 1,
    ...
    DrawTextEx(
        font, text,
        (Vector2){bounds.x + GuiGetStyle(TEXTBOX, TEXT_INNER_PADDING),
                  bounds.y + bounds.height / 2 - fontSize / 2},
        fontSize, 0,
        Fade(GetColor(GuiGetStyle(TEXTBOX, TEXT + (state * 3))), guiAlpha));

    // GuiDrawText(text, GetTextBounds(TEXTBOX, bounds), GuiGetStyle(TEXTBOX,
    // TEXT_ALIGNMENT), Fade(GetColor(GuiGetStyle(TEXTBOX, TEXT + (state*3))),
    // guiAlpha));
    ...
    if (editMode)
        GuiDrawRectangle(
            cursor, 0, BLANK,
            Fade(GetColor(GuiGetStyle(TEXTBOX, BORDER_COLOR_PRESSED)),
                 guiAlpha));
    //--------------------------------------------------------------------
...
bool GuiTextBox(Rectangle bounds, char *text, int textSize, bool editMode) {
    ...
    if ((state != STATE_DISABLED) && !guiLocked) {
        ...
        if (editMode) {
            ...
            if (textAlignment == TEXT_ALIGN_CENTER)
                cursor.x =
                    bounds.x + GetTextWidth(text) / 2 + bounds.width / 2 + 1;
            else if (textAlignment == TEXT_ALIGN_RIGHT)
                cursor.x = bounds.x + bounds.width -
                           GuiGetStyle(TEXTBOX, TEXT_INNER_PADDING);
        } else {
...
bool GuiTextBoxMulti(Rectangle bounds, char *text, int textSize,
    ...
    if ((state != STATE_DISABLED) && !guiLocked) {
        ...
        if (editMode) {
            ...
            int textLength =
                (int)strlen(text);  // Length in bytes (UTF-8 string)
            ...
            if (textLength > 0) {
                if (IsKeyPressed(KEY_BACKSPACE)) {
                    if ((unsigned char)text[textLength - 1] < 127) {
                        ...
                        // Remove latest UTF-8 unicode character introduced (n
                        // bytes)
                        int charUTF8Length = 0;
                        while (((unsigned char)
                                    text[textLength - 1 - charUTF8Length] &
                                0b01000000) == 0)
                            charUTF8Length++;
    ...
    for (int i = 0, codepointLength = 0; text[i] != '\0';
         i += codepointLength) {
        int codepoint = GetCodepoint(text + i, &codepointLength);
        int index =
            GetGlyphIndex(guiFont, codepoint);  // If requested codepoint is not
                                                // found, we get '?' (0x3f)
        ...
        if ((codepointLength == 1) && (codepoint == '\n')) {
            ...
            DrawTextCodepoint(
                guiFont, codepoint, cursorPos,
                (float)GuiGetStyle(DEFAULT, TEXT_SIZE),
                Fade(GetColor(GuiGetStyle(TEXTBOX, TEXT + (state * 3))),
                     guiAlpha));
...
float GuiProgressBar(Rectangle bounds, const char *textLeft,
    ...
    GuiDrawRectangle(
        ...
        Fade(GetColor(GuiGetStyle(PROGRESSBAR, BORDER + (state * 3))),
             guiAlpha),
        BLANK);
    ...
    if ((state == STATE_NORMAL) || (state == STATE_PRESSED))
        GuiDrawRectangle(
            progress, 0, BLANK,
            Fade(GetColor(GuiGetStyle(PROGRESSBAR, BASE_COLOR_PRESSED)),
                 guiAlpha));
    else if (state == STATE_FOCUSED)
        GuiDrawRectangle(
            progress, 0, BLANK,
            Fade(GetColor(GuiGetStyle(PROGRESSBAR, TEXT_COLOR_FOCUSED)),
                 guiAlpha));
    ...
    if (textLeft != NULL) {
        ...
        GuiDrawText(textLeft, textBounds, TEXT_ALIGN_RIGHT,
                    Fade(GetColor(GuiGetStyle(PROGRESSBAR, TEXT + (state * 3))),
                         guiAlpha));
    }
...
void GuiStatusBar(Rectangle bounds, const char *text) {
    ...
    GuiDrawRectangle(
        ...
        Fade(GetColor(GuiGetStyle(STATUSBAR, (state != STATE_DISABLED)
             ...
             guiAlpha));
    GuiDrawText(
        text, GetTextBounds(STATUSBAR, bounds),
    ...
    for (int i = 0; ((i < visibleItems) && (text != NULL)); i++) {
        if (state == STATE_DISABLED) {
            ...
            GuiDrawText(
                ...
                Fade(GetColor(GuiGetStyle(LISTVIEW, TEXT_COLOR_DISABLED)),
                     guiAlpha));
        } else {
            if ((startIndex + i) == itemSelected) {
                // Draw item selected
                ...
                GuiDrawText(
                    text[startIndex + i], GetTextBounds(DEFAULT, itemBounds),
                    GuiGetStyle(LISTVIEW, TEXT_ALIGNMENT),
                    Fade(GetColor(GuiGetStyle(LISTVIEW, TEXT_COLOR_NORMAL)),
                         guiAlpha));
            }
        }
...
Color GuiColorPanel(Rectangle bounds, const char *text, Color color) {
    ...
    pickerSelector.x =
        bounds.x + (float)hsv.y * bounds.width;  // HSV: Saturation
    pickerSelector.y =
                ...
                GuiDrawRectangle(
                    ...
                    ((x + y) % 2)
                        ? Fade(Fade(GetColor(GuiGetStyle(
                                    ...
                                    0.4f),
                               guiAlpha)
                        : Fade(Fade(GetColor(GuiGetStyle(COLORPICKER,
                                                         BASE_COLOR_DISABLED)),
                                    0.4f),
                               guiAlpha));
            }
...
#define RAYGUI_TEXTINPUTBOX_HEIGHT 28
    ...
    if (message != NULL) {
        Vector2 textSize = MeasureTextEx(
            guiFont, message, (float)GuiGetStyle(DEFAULT, TEXT_SIZE), 1);
        ...
        case STATE_NORMAL: {
            if (subdivs > 0) {
                ...
                for (int i = 0; i < linesV; i++) {
                    Rectangle lineV = {bounds.x + spacing * i / subdivs,
                ...
                for (int i = 0; i < linesH; i++) {
                    Rectangle lineH = {bounds.x,
                                       bounds.y + spacing * i / subdivs,
                                       bounds.width, 1};
                    GuiDrawRectangle(
                        lineH, 0, BLANK,
                        ((i % subdivs) == 0)
                    ...
                    case 'f': {
                        ...
                        if (charmapFileName[0] != '0') {
                            ...
                            if (charValues != NULL) {
                                ...
                                int *values = (int *)RAYGUI_MALLOC(glyphCount *
                                                                   sizeof(int));
                                for (int i = 0; i < glyphCount; i++)
                                    values[i] = TextToInteger(chars[i]);

                                if (font.texture.id !=
                                    GetFontDefault().texture.id)
        ...
        if ((signature[0] == 'r') && (signature[1] == 'G') &&
            ...
            for (int i = 0; i < propertyCount; i++) {
                ...
                if (controlId == 0)  // DEFAULT control
                {
                    ...
                    // controls NOTE: All DEFAULT properties should be defined
                    // first in the file
            ...
            // Font loading is highly dependant on raylib API to load font data
            // and image
#if !defined(RAYGUI_STANDALONE)
            ...
            if (fontDataSize > 0) {
                ...
                if (fontImageCompSize < fontImageUncompSize) {
                    ...
                    if (dataUncompSize != fontImageUncompSize)
                        RAYGUI_LOG(
                            "WARNING: Uncompressed font atlas image data could "
                            "be corrupted");
                ...
                // Set font texture source rectangle to be used as white texture
                // to draw shapes NOTE: This way, all gui can be draw using a
                // single draw call
        ...
        if ((signature[0] == 'r') && (signature[1] == 'G') &&
            ...
            if (loadIconsName) {
                ...
                for (int i = 0; i < iconCount; i++) {
                    ...
                    fread(guiIconsName[i], RAYGUI_ICON_MAX_NAME_LENGTH, 1,
                          rgiFile);
                }
            } else
...
#define BIT_CHECK(a, b) ((a) & (1u << (b)))
    ...
    for (int i = 0, y = 0; i < RAYGUI_ICON_SIZE * RAYGUI_ICON_SIZE / 32; i++) {
        for (int k = 0; k < 32; k++) {
            if (BIT_CHECK(guiIcons[iconId * RAYGUI_ICON_DATA_ELEMENTS + i],
                          k)) {
#if !defined(RAYGUI_STANDALONE)
                DrawRectangle(posX + (k % RAYGUI_ICON_SIZE) * pixelSize,
                              posY + y * pixelSize, pixelSize, pixelSize,
                              color);
#endif
            }
...
#define BIT_SET(a, b) ((a) |= (1u << (b)))
    ...
    // For example, in case of 16x16 pixels, every 2 lines fit in one unsigned
    // int data element
...
#define ICON_TEXT_PADDING 4
    ...
    if ((text != NULL) && (text[0] != '\0')) {
        ...
        size = MeasureTextEx(font, text + textIconOffset, fontSize, 0);
        if (textIconOffset > 0)
...
static Rectangle GetTextBounds(int control, Rectangle bounds) {
    ...
    // Consider TEXT_PADDING properly, depends on control type and
    // TEXT_ALIGNMENT
    switch (control) {
        case COMBOBOX:
            ...
            break;
        case VALUEBOX:
            break;  // NOTE: ValueBox text value always centered, text padding
                    // applies to label
        default: {
...
#define ICON_TEXT_PADDING 4
    ...
    if ((text != NULL) && (text[0] != '\0')) {
        int iconId = 0;
        text =
            GetTextIcon(text, &iconId);  // Check text for icon and move cursor
...
#endif
        DrawTextEx(guiFont, text, position,
                   (float)GuiGetStyle(DEFAULT, TEXT_SIZE),
...
static int GuiScrollBar(Rectangle bounds, int value, int minValue,
    ...
    if (isVertical) {
        ...
        sliderSize = (sliderSize >= scrollbar.height)
                         ...
                         : sliderSize;  // Make sure the slider won't get
            ...
            if (IsMouseButtonPressed(MOUSE_LEFT_BUTTON)) {
                ...
                if (!isVertical) {
                    Rectangle scrollArea = {
                        arrowUpLeft.x + arrowUpLeft.width, arrowUpLeft.y,
                        scrollbar.width,
                        bounds.height -
                            2 * GuiGetStyle(SCROLLBAR, BORDER_WIDTH)};
                    if (CheckCollisionPointRec(mousePoint, scrollArea))
                        value = (int)(((float)(mousePoint.x - scrollArea.x -
                                               slider.width / 2) *
                                       range) /
    ...
    *bytesProcessed = 1;
    ...
    if (octet <= 0x7f) {
        ...
        if ((octet >= 0xe0) && (0 <= 0xef)) {
            code = ((octet & 0xf) << 12) | ((octet1 & 0x3f) << 6) |
                   (octet2 & 0x3f);
            *bytesProcessed = 3;
        }
